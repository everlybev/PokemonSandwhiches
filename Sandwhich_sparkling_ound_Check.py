...
##                        found_line = True
    if not False:
        file = 'Recipe_List-3_Ingredient_Sparkling.txt'
        file_object = open(file, 'r')
        recipess = file_object.read()
        file_object.close()
        all_combos_found = True
        desired_third_powers = []
        for Power in powers_names_list:
            if Power == 'egg':
                pass
            elif Power == 'sparkling':
                pass
            elif Power == 'title':
                pass
            else:
                for PokeType in types_names_list:
                    desired_third_powers.append(Power + ', 3, ' + PokeType)
        print(len(desired_third_powers))
        #powers_names_list types_names_list
        for test in desired_third_powers:
            if recipess.__contains__(test):
                print(test)
            else:
